...
/**
 * Cached GoogleGenAI instance to ensure we initialize once and reuse, 
 * but lazily to pick up the most current environment variables.
 */
...
const getAIClient = () => {
  ...
  // If no instance exists or the key in the environment has changed, update the singleton.
  if (!aiInstance || cachedKey !== apiKey) {
    cachedKey = apiKey;
...
export const generatePersonaImage = async (city: string, age: number, gender: Gender): Promise<string> => {
  ...
  try {
    ...
    if (!parts) throw new Error("Empty response from AI studio.");
    ...
    throw new Error("No fashion visual was generated. Please try again.");
  } catch (error: any) {
    console.error("Gemini Service Error:", error);
    // Preserving "Requested entity was not found" message for App.tsx UI handling
    if (error.message?.includes("Requested entity was not found")) {
      throw new Error("Requested entity was not found. Please ensure you have selected a valid API key with billing enabled.");
    }
    throw error;
  }
};
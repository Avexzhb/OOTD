 ...
 * Note: This captures the current process.env.API_KEY.
 */
...
export const generatePersonaImage = async (city: string, age: number, gender: Gender): Promise<string> => {
  ...
  try {
    ...
    if (response.candidates && response.candidates[0].content.parts) {
      // Find the image part as the response may contain both text and image parts
    ...
    // Preserving "Requested entity was not found" wording so App.tsx can correctly prompt for API key reset

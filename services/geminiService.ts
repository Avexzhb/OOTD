...
export const generatePersonaImage = async (city: string, age: number, gender: Gender): Promise<string> => {
  // Initialize with the exact syntax required for production environment variables
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
  
  // Construct a sophisticated prompt optimized for Imagen 4.0
  ...
  try {
    ...
    if (response.generatedImages && response.generatedImages.length > 0) {
      ...
      throw new Error("The AI style studio is currently busy. Please try again.");
    }
  } catch (error) {
